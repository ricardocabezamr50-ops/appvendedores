...
const FILTER_VALUE: string | null = 'material';
...
export default function MaterialScreen() {
  ...
  if (level === null || loading) return (<View style={S.center}><ActivityIndicator /><Text style={S.muted}>Cargando material…</Text></View>);
    ...
    <View style={S.container}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={S.card}>
            <View style={S.row}>
              {item.thumbnailUrl ? <Image source={{ uri: item.thumbnailUrl }} style={S.thumb} /> : <View style={[S.thumb, S.thumbFallback]}><Text>🖼️</Text></View>}
              <View style={{ flex: 1 }}>
